...
class VsGenerator : public Generator {
	bool testMode;
	inline bool generateFilters(jinn::model::Project* p, futil::util::FileList&);
...
public:
	VsGenerator(bool testMode) : testMode(testMode) {}

	virtual void generate(model::GlobalScope*);

	const char* vsVersion;
